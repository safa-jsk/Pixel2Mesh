...
from pixel2mesh.utils.vis.renderer import MeshRenderer
...
class Evaluator(CheckpointRunner):
    ...
    def init_fn(self, shared_model=None, **kwargs):
        ...
        if shared_model is not None:
            ...
            if self.model_on_gpu:
                if len(self.gpus) == 0:
                    raise ValueError("model_on_gpu=True but no GPUs are configured. Set --gpus to at least 1.")
                self.logger.info("Design A GPU: Model will run on GPU, chamfer/renderer on GPU")
                if len(self.gpus) > 1:
                    self.model = torch.nn.DataParallel(self.model, device_ids=self.gpus).cuda()
                else:
                    self.model = self.model.cuda()
            else:
                # [DESIGN.A][STAGE.MODEL_INFERENCE] Design A CPU baseline
                self.logger.info("Design A CPU: Model will run on CPU, chamfer/renderer on GPU")
    ...
    def evaluate_step(self, input_batch):
        ...
        with torch.inference_mode():
            ...
            if self.options.model.name == "pixel2mesh":
                # Ensure FP32 for metric computation; move to CUDA for chamfer kernel
                pred_vertices = out["pred_coord"][-1].float()
                if not pred_vertices.is_cuda:
                ...
                elif torch.is_tensor(gt_points):
                    gt_points = gt_points.cuda().float()
                self.evaluate_chamfer_and_f1(pred_vertices, gt_points, input_batch["labels"])
            elif self.options.model.name == "classifier":
                labels = input_batch["labels"]
                if self.model_on_gpu:
                    labels = labels.cuda(non_blocking=True)
                self.evaluate_accuracy(out, labels)
    ...
    def _build_eval_dataloader(self):
        ...
        if self.model_on_gpu and self.options.num_workers > 0:
            loader_kwargs["multiprocessing_context"] = "spawn"
            self.logger.info("Using DataLoader multiprocessing_context='spawn' for CUDA-safe workers")
    ...
    def evaluate(self):
        ...
        test_data_loader = self._build_eval_dataloader()
        ...
        if self.warmup_iters > 0 and self.model_on_gpu and torch.cuda.is_available():
